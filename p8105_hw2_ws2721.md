...
``` r
bakes$baker = gsub('"Jo"', 'Jo', bakes$baker)
```

``` r
results$baker = gsub('Joanne','Jo', results$baker)
```
...
There are total of 40 titles of “Winner” and “Star Baker” awarded to 20
distinct bakers in between series 5 and 10. I could see that the most of
the winners have low technical level that is around 1 to 2. It could be
surprising to see the baking that are easy to make are the winning
bakes. Also, generally people who won the STAR BAKER prior seems like to
...
viewers that are associated with. The series were chr variable so I had
to change it to double in order to filter for future average
calculation.
